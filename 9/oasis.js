...
const processSequence = (sequence) => {
  ...
  for (let i = decompositions.length - 1; i >= 0; i--) {
    const decomposition = decompositions[i];
    decomposition[decomposition.length] = decompositions[i + 1] ? decompositions[i + 1].at(-1) + decompositions[i].at(-1) : decompositions[i].at(-1);
  }
  
  return decompositions[0].at(-1);
}
